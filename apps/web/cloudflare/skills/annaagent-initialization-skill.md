# AnnaAgent Initialization Skill
...
3. 不能生成来访者主动开场白，等待咨询师先发起对话。
4. 输出必须是严格 JSON，不得包含 Markdown 或额外说明。
...
3. 分析 `style`
4. 生成 `complaint_chain`
5. 推断 `previous_scales`
6. 推断 `current_scales`
7. 归纳 `status`
8. 推断 `current_emotion`
9. 生成 `system_prompt`
...
3. `complaint_chain` 至少3条，必须体现由表及里的递进。
4. `previous_scales` 必须包含 `p_bdi`、`p_ghq`、`p_sass`。
5. `current_scales` 必须包含 `bdi`、`ghq`、`sass`。
6. 量表值必须是 `A/B/C/D` 数组：
   - `p_bdi`: 21 项
   - `p_ghq`: 28 项
   - `p_sass`: 21 项
   ...
   - `ghq`: 28 项
   - `sass`: 21 项
7. `system_prompt` 必须是完整、可直接用于 seeker 生成的系统提示词。
8. 不要生成 `sample_statements` 或任何候选开场白。
