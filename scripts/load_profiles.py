...
async def load_from_huggingface():
    ...
    async with async_session() as session:
        ...
        existing_count = (
            await session.execute(select(func.count()).select_from(SeekerProfile))
        ).scalar() or 0
        ...
        for group_key in [
            ...
            count = (
                await session.execute(
                    select(func.count())
                    .select_from(SeekerProfile)
                    .where(SeekerProfile.group_tag == group_key)
                )
            ).scalar() or 0
...
async def seed_knowledge():
    ...
    async with async_session() as session:
        ...
        existing = (
            await session.execute(select(func.count()).select_from(KnowledgeItem))
        ).scalar() or 0
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Load data into annapod database")
    parser.add_argument(
        "--source", choices=["huggingface", "local"], default="huggingface"
    )
    parser.add_argument("--path", type=str, help="Path to local JSON file")
    parser.add_argument(
        "--seed-knowledge", action="store_true", help="Also seed knowledge base"
    )
